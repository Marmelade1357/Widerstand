...
const MIN_PLAYERS = 5;
const SKIP_MS = 500;
const ENV = { SKIP_MIN_WAIT_MS: String(SKIP_MS), HOST_HANDOVER_MS: '400' };
...
async function setup(url) {
  ...
  host.emit('setPlotCardsEnabled', { enabled: false }); host.emit('setCommanderEnabled', { enabled: false });
  for (let i = 2; i < MIN_PLAYERS; i++) host.emit('addBot');
  await waitForState(host, (s) => s.players.length >= MIN_PLAYERS);
  host.emit('startGame');
  host.emit('ready'); guest.emit('ready');
...
async function testSkip() {
  ...
  try {
    ...
    if (st.waiting.ids.length > 1) { host.emit('vote', { approve: true }); }
